...
namespace BiometricsClient
{
         delegate void FunctionCall(dynamic param);

        public class DPLibrary : DPFP.Capture.EventHandler
        {
        ...
        public DPLibrary(Form _frm, Label _StatusLine, TextBox _Prompt, TextBox _StatusText, PictureBox _Picture, string _FingerTable, bool _EnrollmentMode = true, bool _UseIdentityProcessing = false)
        {
            try
            {
                ...
                if (UseIdentityProcessing & EnrollmentMode == true)
                    throw new Exception("Invalid Operation, Required for Identification Only");
        ...
        public virtual Bitmap Process(DPFP.Sample Sample)
        {
            try
            {
            ...
            public void MakeReport(string status)
            {
            ...
            public string UpdateStatus(int FAR = 0)
            {
                // Show number of samples needed.
                if (EnrollmentMode)
                    return string.Format("Fingerprint samples needed: {0}", Enroller.FeaturesNeeded);
                else
                    return string.Format("False Accept Rate (FAR) = {0}", FAR);
            }
            ...
            public byte[] ConvertRawBmpAsTemplateData(Bitmap RawBmp, short VertDpi = 700, short HorDpi = 700)
            {
                VariantConverter VConverter;
                Enroller = new DPFP.Processing.Enrollment();
                RawBmp = EncodeBitmap(RawBmp, VertDpi, HorDpi);
